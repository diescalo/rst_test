...
class XBee16BitAddress(object):
    ...
    | Attributes:
    |     **COORDINATOR_ADDRESS** (XBee16BitAddress): 16-bit address reserved 
                                                      for the coordinator.
    |     **BORADCAST_ADDRESS** (XBee16BitAddress): 16-bit broadcast address.
    |     **UNKNOWN_ADDRESS** (XBee16BitAddress): 16-bit unknown address.
    |     **PATTERN** (String): Pattern for the 16-bit address string: 
                                ``(0[xX])?[0-9a-fA-F]{1,4}``
    """
    ...
    def from_hex_string(cls, address):
        ...
        |     address (String): String containing the address. Must be made by
                                hex. digits without blanks. Minimum 1 character,
                                maximum 4 (16-bit).

        | Raises:
        |     ValueError:
        |         1. If ``address`` has less than 1 character.
        |         2. If ``address`` contains non-hexadecimal characters.
        """
        if len(address) < 1:
...
class XBee64BitAddress(object):
    ...
    def from_hex_string(cls, address):
        ...
        | Raises:
        |     ValueError: If the address's length is less than 1 or does not math
    ...
    def from_bytes(cls, *args):
        ...
        | Args:
        |     args (8 Integers): 8 integers that represents the bytes 1 to 8 of
    ...
    def __eq__(self, other):
        ...
        Returns:
            Boolean: ``True`` if self and other have the same value and type, ``False`` in
                     other case.
        """
        if other is None:
            return False
...
class XBeeIMEIAddress(object):
    ...
    def __init__(self, address):
        ...
        | Raises:
        |     ValueError:
        |         1. If ``address`` is ``None``.
        |         2. If length of ``address`` greater than 8.
        """
        if address is None:
    ...
    def from_string(cls, address):
        ...
        |     ValueError:
        |         1. If ``address`` is ``None``.
        |         2. If ``address`` does not math the pattern: ``^\d{0,15}$``.
        """
        if address is None:
            raise ValueError("IMEI address cannot be None")
        if not (cls.__REGEXP.match(address)):
