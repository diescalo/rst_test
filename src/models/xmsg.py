...
class XBeeMessage(object):
    ...
    def __get_remote_device(self):
        """
        | Returns:
        |     :class:`.RemoteXBeeDevice`: the device which has sent
                                          the message.
        """
    ...
    def __get_timestamp(self):
        ...
        | Returns:
        |     Float: the returned value of using :meth:`time.clock()` function when the
...
class ExplicitXBeeMessage(XBeeMessage):
    """
    This class represents an Explicit XBee message, which is formed by 
    all parameters of a common XBee message and:
...
class IPMessage(object):
    ...
    def __init__(self, ip_addr, source_port, dest_port, protocol, data):
        ...
        |         3. If ``data`` is ``None``.
        |         4. If ``source_port`` is less than 0 or greater than 65535.
        |         5. If ``dest_port`` is less than 0 or greater than 65535.
        """
        if ip_addr is None:
