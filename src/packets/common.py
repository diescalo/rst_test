...
from src.models.xaddr import XBee16BitAddress, XBee64BitAddress
from src.models.xstatus import ATCommandStatus, DiscoveryStatus, TransmitStatus, ModemStatus
...
class ATCommPacket(XBeeAPIPacket):
    ...
    def __init__(self, frame_id, command, parameter=None):
        ...
        |         2. If length of ``command`` is different than 2.
        ...
        XBeeAPIPacket.__init__(self, ApiFrameType.AT_COMMAND)
    ...
    def create_packet(raw, operating_mode):
        ...
        |         1. If the bytearray length is less than 6. (start delim. + length (2 bytes) + frame type + frame id +
                     checksum = 6 bytes).
        ...
        |         6. If the frame type is different than :attr:`.ApiFrameType.AT_COMMAND`
...
class ATCommResponsePacket(XBeeAPIPacket):
    ...
    def __init__(self, frame_id, command, response_status=ATCommandStatus.OK, comm_value=None):
        ...
        XBeeAPIPacket.__init__(self, ApiFrameType.AT_COMMAND_RESPONSE)
        self._frame_id = frame_id
        self.__command = command
    ...
    def create_packet(raw, operating_mode):
        ...
        |         4. If the calculated checksum is different than the checksum field value (last byte).
        |         5. If operating_mode mode is not supported.
        |         6. If the frame type is different than :attr:`.ApiFrameType.AT_COMMAND_RESPONSE`.
        |         7. If the command status field is not a valid value. See :class:`.ATCommandStatus`
...
class ReceivePacket(XBeeAPIPacket):
    ...
    def __init__(self, x64bit_addr, x16bit_addr, receive_options, rf_data=None):
        ...
        |     :class:`.XBeeAPIPacket`
        """
        XBeeAPIPacket.__init__(self, ApiFrameType.RECEIVE_PACKET)
    ...
    def create_packet(raw, operating_mode):
        ...
        | Raises:
        |     InvalidPacketException:
        |         1. If the bytearray length is less than 16. (start delim. + length (2 bytes) + frame type + frame id +
                     64bit addr. + 16bit addr. + Receive options + checksum = 16 bytes).
        |         2. If the length field of 'raw' is different than its real length. (length field: bytes 2 and 3)
        |         3. If the first byte of 'raw' is not the header byte. See :class:`.SpecialByte`.
        |         4. If the calculated checksum is different than the checksum field value (last byte).
        |         5. If operating_mode mode is not supported.
        |         6. If the frame type is not :attr:`.ApiFrameType.RECEIVE_PACKET`.
...
class RemoteATCommandPacket(XBeeAPIPacket):
    ...
    def __init__(self, frame_id, x64bit_addr, x16bit_addr, transmit_options, command, parameter=None):
        ...
        XBeeAPIPacket.__init__(self, ApiFrameType.REMOTE_AT_COMMAND_REQUEST)
        self._frame_id = frame_id
        self.__x64bit_addr = x64bit_addr
        self.__x16bit_addr = x16bit_addr
    ...
    def create_packet(raw, operating_mode):
        ...
        | Raises:
        |     InvalidPacketException:
        |         1. If the Bytearray length is less than 19. (start delim. + length (2 bytes) + frame type + frame id +
                     64bit addr. + 16bit addr. + transmit options + command (2 bytes) + checksum = 19 bytes).
        |         2. If the length field of 'raw' is different than its real length. (length field: bytes 2 and 3)
        |         3. If the first byte of 'raw' is not the header byte. See :class:`.SpecialByte`.
        |         4. If the calculated checksum is different than the checksum field value (last byte).
        |         5. If operating_mode mode is not supported.
        |         6. If the frame type is not :attr:`.ApiFrameType.REMOTE_AT_COMMAND_REQUEST`.
...
class RemoteATCommandResponsePacket(XBeeAPIPacket):
    ...
    def __init__(self, frame_id, x64bit_addr, x16bit_addr, command, response_status, comm_value=None):
        ...
        XBeeAPIPacket.__init__(self, ApiFrameType.REMOTE_AT_COMMAND_RESPONSE)
        self._frame_id = frame_id
        self.__x64bit_addr = x64bit_addr
        self.__x16bit_addr = x16bit_addr
        self.__command = command
    ...
    def create_packet(raw, operating_mode):
        ...
        |     InvalidPacketException:
        |         1. If the bytearray length is less than 19. (start delim. + length (2 bytes) + frame type + frame id +
                     64bit addr. + 16bit addr. + receive options + command (2 bytes) + checksum = 19 bytes).
        |         2. If the length field of 'raw' is different than its real length. (length field: bytes 2 and 3)
        |         3. If the first byte of 'raw' is not the header byte. See :class:`.SpecialByte`.
        |         4. If the calculated checksum is different than the checksum field value (last byte).
        |         5. If operating_mode mode is not supported.
        |         6. If the frame type is not :attr:`.ApiFrameType.REMOTE_AT_COMMAND_RESPONSE`.
...
class TransmitPacket(XBeeAPIPacket):
    ...
    def __init__(self, frame_id, x64bit_addr, x16bit_addr, broadcast_radius, transmit_options, rf_data=None):
        ...
        XBeeAPIPacket.__init__(self, ApiFrameType.TRANSMIT_REQUEST)
        self._frame_id = frame_id
        self.__x64bit_addr = x64bit_addr
        self.__x16bit_addr = x16bit_addr
    ...
    def create_packet(raw, operating_mode):
        ...
        | Raises:
        |     InvalidPacketException:
        |         1. If the bytearray length is less than 18. (start delim. + length (2 bytes) + frame type + frame id +
                     64bit addr. + 16bit addr. + Receive options + checksum = 16 bytes).
        |         2. If the length field of 'raw' is different than its real length. (length field: bytes 2 and 3)
        |         3. If the first byte of 'raw' is not the header byte. See :class:`.SpecialByte`.
        |         4. If the calculated checksum is different than the checksum field value (last byte).
        |         5. If operating_mode mode is not supported.
        |         6. If the frame type is not :attr:`.ApiFrameType.TRANSMIT_REQUEST`.
...
class TransmitStatusPacket(XBeeAPIPacket):
    ...
    def __init__(self, frame_id, x16bit_addr, transmit_retry_count, transmit_status=TransmitStatus.SUCCESS,
        ...
        XBeeAPIPacket.__init__(self, ApiFrameType.TRANSMIT_STATUS)
        self._frame_id = frame_id
        self.__x16bit_addr = x16bit_addr
    ...
    def create_packet(raw, operating_mode):
        ...
        | Raises:
        |     InvalidPacketException:
        |         1. If the bytearray length is less than 11. (start delim. + length (2 bytes) + frame type + frame id +
                     16bit addr. + transmit retry count + delivery status + discovery status + checksum = 11 bytes).
        |         2. If the length field of 'raw' is different than its real length. (length field: bytes 2 and 3)
        |         3. If the first byte of 'raw' is not the header byte. See :class:`.SpecialByte`.
        |         4. If the calculated checksum is different than the checksum field value (last byte).
        |         5. If operating_mode mode is not supported.
        |         6. If the frame type is not :attr:`.ApiFrameType.TRANSMIT_STATUS`.
...
class ModemStatusPacket(XBeeAPIPacket):
    ...
    def __init__(self, modem_status):
        ...
        |     :class:`.XBeeAPIPacket`
        """
        XBeeAPIPacket.__init__(self, ApiFrameType.MODEM_STATUS)
    ...
    def create_packet(raw, operating_mode):
        ...
        |         4. If the calculated checksum is different than the checksum field value (last byte).
        |         5. If operating_mode mode is not supported.
        |         6. If the frame type is not :attr:`.ApiFrameType.MODEM_STATUS`.
...
class IODataSampleRxIndicatorPacket(XBeeAPIPacket):
    ...
    def __init__(self, x64bit_addr, x16bit_addr, receive_options, rf_data=None):
        ...
        |     :class:`.XBeeAPIPacket`
        """
        XBeeAPIPacket.__init__(self, ApiFrameType.IO_DATA_SAMPLE_RX_INDICATOR)
        self.__x64bit_addr = x64bit_addr
        self.__x16bit_addr = x16bit_addr
        self.__receive_options = receive_options
        self.__rf_data = rf_data
    ...
    def create_packet(raw, operating_mode):
        ...
        |         1. If the bytearray length is less than 20. (start delim. + length (2 bytes) + frame type +
                     64bit addr. + 16bit addr. + rf data (5 bytes) + checksum = 20 bytes).
        |         2. If the length field of 'raw' is different than its real length. (length field: bytes 2 and 3)
        |         3. If the first byte of 'raw' is not the header byte. See :class:`.SpecialByte`.
        |         4. If the calculated checksum is different than the checksum field value (last byte).
        |         5. If operating_mode mode is not supported.
        |         6. If the frame type is not :attr:`.ApiFrameType.IO_DATA_SAMPLE_RX_INDICATOR`.
...
class ExplicitAddressingPacket(XBeeAPIPacket):
    ...
    def __init__(self, frame_id, x64bit_addr, x16bit_addr, source_endpoint, dest_endpoint, cluster_id,
        ...
        | Raises:
        |     ValueError:
        |         1. If ``frame_id``, ``src_endpoint`` or ``dst_endpoint`` are less than 0 or greater than 255.
        |         2. If lengths of ``cluster_id`` or ``profile_id`` (respectively) are less than 0 or greater than
                     0xFFFF.
        ...
        XBeeAPIPacket.__init__(self, ApiFrameType.EXPLICIT_ADDRESSING)
        self._frame_id = frame_id
    ...
    def create_packet(raw, operating_mode):
        ...
        | Raises:
        |     InvalidPacketException:
        |         1. If the bytearray length is less than 24. (start delim. + length (2 bytes) + frame type +
                     frame ID + 64bit addr. + 16bit addr. + source endpoint + dest. endpoint + cluster ID (2 bytes) +
                     profile ID (2 bytes) + broadcast radius + transmit options + checksum = 24 bytes).
        |         2. If the length field of 'raw' is different than its real length. (length field: bytes 2 and 3)
        |         3. If the first byte of 'raw' is not the header byte. See :class:`.SpecialByte`.
        |         4. If the calculated checksum is different than the checksum field value (last byte).
        |         5. If operating_mode mode is not supported.
        |         6. If the frame type is different than :attr:`.ApiFrameType.EXPLICIT_ADDRESSING`
...
class ExplicitRXIndicatorPacket(XBeeAPIPacket):
    ...
    def __init__(self, x64bit_addr, x16bit_addr, source_endpoint,
        ...
        | Raises:
        |     ValueError:
        |         1. If ``src_endpoint`` or ``dst_endpoint`` are less than 0 or greater than 255.
        |         2. If lengths of ``cluster_id`` or ``profile_id`` (respectively) are different than 2.
        ...
        XBeeAPIPacket.__init__(self, ApiFrameType.EXPLICIT_RX_INDICATOR)
        self.__x64bit_addr = x64bit_addr
        self.__x16bit_addr = x16bit_addr
        self.__source_endpoint = source_endpoint
        self.__dest_endpoint = dest_endpoint
    ...
    def create_packet(raw, operating_mode):
        ...
        | Raises:
        |     InvalidPacketException:
        |         1. If the bytearray length is less than 22. (start delim. + length (2 bytes) + frame type +
                     64bit addr. + 16bit addr. + source endpoint + dest. endpoint + cluster ID (2 bytes) +
                     profile ID (2 bytes) + receive options + checksum = 22 bytes).
        |         2. If the length field of 'raw' is different than its real length. (length field: bytes 2 and 3)
        |         3. If the first byte of 'raw' is not the header byte. See :class:`.SpecialByte`.
        |         4. If the calculated checksum is different than the checksum field value (last byte).
        |         5. If operating_mode mode is not supported.
        |         6. If the frame type is different than :attr:`.ApiFrameType.EXPLICIT_RX_INDICATOR`
