...
from src.xexc import InvalidPacketException, InvalidOperatingModeException
...
class XBeePacket:
    ...
    Derived classes should implement their own payload generation depending 
    on their type.
    ...
    def _escape_data(data):
        ...
        Args:
            data (Bytearray): the byterray to escape.
    ...
    def _unescape_data(data):
        """
        Unescapes the provided bytearray data.
...
class XBeeAPIPacket(XBeePacket):
    ...
    def _check_api_packet(raw, min_length=5):
        ...
        |         1. If the bytearray length is less than 5. (start delim. + length (2 bytes) + frame type +
                     checksum = 5 bytes).
...
class GenericXBeePacket(XBeeAPIPacket):
    ...
    def __init__(self, rf_data):
        ...
        |     :class:`.XBeeAPIPacket`
        """
        XBeeAPIPacket.__init__(self, api_frame_type=ApiFrameType.GENERIC)
    ...
    def create_packet(raw, operating_mode=OperatingMode.API_MODE):
        ...
        |         6. if the frame type is different than :attr:`.ApiFrameType.GENERIC`.
