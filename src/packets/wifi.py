...
from src.models.xstatus import ATCommandStatus
...
class IODataSampleRxIndicatorWifiPacket(XBeeAPIPacket):
    ...
    def __init__(self, source_address, rssi, receive_options, rf_data=None):
        ...
        |     :class:`.XBeeAPIPacket`
        """
        XBeeAPIPacket.__init__(self, ApiFrameType.IO_DATA_SAMPLE_RX_INDICATOR_WIFI)
    ...
    def create_packet(raw, operating_mode):
        ...
        |         1. If the bytearray length is less than 16. (start delim. + length (2 bytes) + frame type +
                     source addr. (4 bytes) + rssi + receive options + rf data (5 bytes) + checksum = 16 bytes).
        ...
        |         6. If the frame type is not :attr:`.ApiFrameType.IO_DATA_SAMPLE_RX_INDICATOR_WIFI`.
...
class RemoteATCommandWifiPacket(XBeeAPIPacket):
    ...
    def __init__(self, frame_id, dest_address, transmit_options, command, parameter=None):
        ...
        XBeeAPIPacket.__init__(self, ApiFrameType.REMOTE_AT_COMMAND_REQUEST_WIFI)
    ...
    def create_packet(raw, operating_mode):
        ...
        | Raises:
        |     InvalidPacketException:
        |         1. If the Bytearray length is less than 17. (start delim. + length (2 bytes) + frame type + frame id +
                     dest. addr. (8 bytes) + transmit options + command (2  bytes) + checksum = 17 bytes).
        |         2. If the length field of 'raw' is different than its real length. (length field: bytes 2 and 3)
        |         3. If the first byte of 'raw' is not the header byte. See :class:`.SpecialByte`.
        |         4. If the calculated checksum is different than the checksum field value (last byte).
        |         5. If operating_mode mode is not supported.
        |         6. If the frame type is not :attr:`.ApiFrameType.REMOTE_AT_COMMAND_REQUEST_WIFI`.
...
class RemoteATCommandResponseWifiPacket(XBeeAPIPacket):
    ...
    def __init__(self, frame_id, source_address, command, response_status, comm_value=None):
        ...
        XBeeAPIPacket.__init__(self, ApiFrameType.REMOTE_AT_COMMAND_RESPONSE_WIFI)
        self._frame_id = frame_id
        self.__source_address = source_address
        self.__command = command
    ...
    def create_packet(raw, operating_mode):
        ...
        |     InvalidPacketException:
        |         1. If the bytearray length is less than 17. (start delim. + length (2 bytes) + frame type + frame id +
                     source addr. (8 bytes) +  command (2 bytes) + receive options + checksum = 17 bytes).
        |         2. If the length field of 'raw' is different than its real length. (length field: bytes 2 and 3)
        |         3. If the first byte of 'raw' is not the header byte. See :class:`.SpecialByte`.
        |         4. If the calculated checksum is different than the checksum field value (last byte).
        |         5. If operating_mode mode is not supported.
        |         6. If the frame type is not :attr:`.ApiFrameType.REMOTE_AT_COMMAND_RESPONSE_WIFI`.
